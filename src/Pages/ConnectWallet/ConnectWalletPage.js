...
export default function ConnectWalletPage() {
    ...
    const updateWalletAddress = async(walletAddressString)=>{
        ...
        const response = await userWalletAddress(userDetails.userDetails.telegramId, data);
    }
    ...
    return (
        <React.Fragment>
            <main className="container">
                {tonWalletAddress ? (
                    <div>
                        <p className="wallet-address">Connected: {formatAddress(tonWalletAddress)}</p>
                        <button className="connectWallet-button" onClick={handleWalletAction}>
                            Disconnect Wallet
                        </button>
                    </div>
                ) : (
